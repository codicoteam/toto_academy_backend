...
router.post("/signup", async (req, res) => {
  try {
    ...
    // Create the new student (wallet is ALWAYS created inside this function)
    const newStudent = await studentService.createStudent(studentData);

    // Generate and send OTP
    try {
      const otpResult = await studentService.generateAndSendOTP(
      ...
      return res.status(201).json({
        message:
          "Student registered successfully. Wallet created. OTP sent to your phone number. Please verify.",
        data: newStudent,
        token,
        otpInfo: {
          success: true,
      ...
      // OTP sending failed, but student + wallet were created
      const token = jwt.sign(
        { id: newStudent._id, email: newStudent.email },
        "toto_academy_2025",
        { expiresIn: "96h" }
      ...
      return res.status(201).json({
        message:
          "Student registered successfully and wallet created, but OTP could not be sent.",
        data: newStudent,
        token,
        otpInfo: {
          success: false,
        ...
        warning:
          "OTP sending failed. Please request OTP manually later for phone verification.",
      });
    }
  } catch (error) {
...
router.get("/pending-profile-pictures", authenticateToken, async (req, res) => {
  try {
    const students =
      await studentService.getStudentsWithPendingProfilePictures();
    res.status(200).json({
...
router.put(
  "/approve-profile-picture/:id",
  authenticateToken,
  async (req, res) => {
    try {
      const studentId = req.params.id;
      const updatedStudent = await studentService.approveProfilePicture(
        studentId
      );
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const updatedStudent = await studentService.rejectProfilePicture(
        studentId,
        rejection_reason
      );
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      res.status(200).json({
        message:
          "Profile picture updated successfully and submitted for approval",
        data: updatedStudent,
      });
    } catch (error) {
      res.status(400).json({
